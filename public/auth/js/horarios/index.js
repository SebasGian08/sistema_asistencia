var $dataTableHorarios, $dataTable;
$(function(){

    const $table = $("#tableHorarios");

    $dataTableHorarios = $table.DataTable({
        ...
        "buttons": [],
        "ajax": {
            url: "/auth/horarios/list_all"
        },
        "columns": [
            { title: "ID", data: "id", className: "text-center" },
            { title: "Ingreso ", data: "ingreso"},
            { title: "Salida ", data: "salida"},
            {
                data: null,
                defaultContent:
    ...
    $table.on("click", ".btn-delete", function () {
        const id = $dataTableHorarios.row($(this).parents("tr")).data().id;
        const formData = new FormData();
        formData.append('_token', $("input[name=_token]").val());
        formData.append('id', id);
        confirmAjax(`/auth/horarios/delete`, formData, "POST", null, null, function () {
            $dataTableHorarios.ajax.reload(null, false);
        });
    });
